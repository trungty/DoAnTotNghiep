...
<?php
	...
	if(isset($_SESSION['idadmin']))
	{
		...
		{
			$casy=$_POST['casy'];
			$sql=mysqli_query($link,"select * from casy where casy='$casy'");
			...
			{
				mysqli_query($link,"insert into casy(casy) values('$casy')");
...
<div class="workplace">
			...
			<div class="row-form">
				<table width="348" border="0">
					<tr>
						...
						<td width="8">&nbsp;</td>
						<td width="127"><input class="btn btn-large" type="reset" value="Nhập" onClick="return confirm(Làm lại);"></td>
					</tr>
				</table>
				<div class="clear"></div>
			</div>
	...
	<?php
		if($_POST['sua'])
		{
			...
			$sql=mysqli_query($link,"select * from casy where casy='$casymoi'");
			$row=mysqli_num_rows($sql);
			if($row!=0)
			{					
				echo '<script type="text/javascript"> alert("Da Co Ca Sy Nay Trong CSDL"); </script>';
			...
			{
				$update= mysqli_query($link,"update casy set casy='$casymoi' where id IN ($casycu)");
		...
		if($_POST['no'])
		{
			...
			if($casycu=="")
			{
				echo '<script type="text/javascript"> alert("Da Het Ca Sy De Xoa"); </script>';
			}
			else
			{
				mysqli_query($link,"DELETE FROM casy WHERE id IN($casycu)");
			...
			<div class="row-form">
				...
				<div class="span6">
					<select name="casycu" id="s2_1" style="width: 100%;">
						<?php
							$luachon=mysqli_query($link,"select * from casy");
